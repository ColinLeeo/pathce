...
mod tests {
    ...
    #[test]
    fn test_bisect_left_double() {
        let a = vec![1.0, 3.0, 5.0, 7.0, 9.0];
        assert_eq!(bisect_left_double(&a, 0.0), 0);
        assert_eq!(bisect_left_double(&a, 2.0), 1);
        assert_eq!(bisect_left_double(&a, 5.0), 2);
        assert_eq!(bisect_left_double(&a, 10.0), 5);
    }

    #[test]
    fn test_calculate_bins_exponential() {
        let bins = calculate_bins_exponential(100, 5);
        assert!(!bins.is_empty());
        assert_eq!(bins[bins.len() - 1], 100);
    }

    #[test]
    fn test_pointwise_function_mult() {
        let func1 = PiecewiseConstantFunction {
            constants: vec![2.0, 3.0],
            right_interval_edges: vec![5.0, 10.0],
            cumulative_rows: vec![10.0, 25.0],
        };

        let func2 = PiecewiseConstantFunction {
            constants: vec![1.0, 2.0],
            right_interval_edges: vec![3.0, 10.0],
            cumulative_rows: vec![3.0, 17.0],
        };

        let result = pointwise_function_mult(&[func1, func2]);

        assert!(!result.constants.is_empty());
        assert!(!result.right_interval_edges.is_empty());
    ...
    #[test]
    fn test_pointwise_function_mult_empty() {
        let result = pointwise_function_mult(&[]);
        assert!(result.constants.is_empty());
        assert!(result.right_interval_edges.is_empty());
        assert!(result.cumulative_rows.is_empty());
    }
